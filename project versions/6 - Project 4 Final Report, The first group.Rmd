...
***
Table 2 shows some statistics for the variable total credit limit on all credit cards owned. Our total sample is 10,314. We can observe from Figure 2 that the distribution of this chosen response variable is extremely right-skewed and, as a result, contains many outlying observations.
 
The distribution of the predictor, registered retirement income funds (Figure 3), the confounder, after-tax income of the household (Figure 4), and the sampling indicator, net worth of the family unit (Figure 5) also face the same problem of severe right-skewness and influential outliers. 
...
Figure 6, 7, and 8 shows the scatterplot of our variable of interest, total credit limit against the predictor, confounder, and sampling indicator, respectively. Specifically, while figure 6 only shows a slight positive correlation between the total credit limit and the RRIFs, both figures 7 and 8 present that the total credit limit is more strongly correlated to after-tax income and net worth of the family unit. 

Since the stratification by provinces was involved in the collection of samples for this dataset, we will look at the total credit limit distribution by provinces in figures 9.1 and 9.2. Figure 9.1 tells us that samples collected from every province are right-skewed with many outliers. This can be better observed in figure 9.2: the lines of best fit for each province seem to have similar slopes, with some places (such as ON, BC, AB, or QC) having more high-leverage points.
...
Since the exploratory part shows that the distribution of our Y variable is extremely right-skewed and has many influential outliers, we have decided that it is in our best interest to transform the data to tackle this problem. We also recognize the danger of overfitting, so we will not be using box-cox to optimize the transformation (for this set of data), but rather go with a more “natural” type of transformation: taking the log.
...
We have decided to keep our predictor element linear due to the fact that this chosen variable contains too many 0-values and thus is not suitable for the use of other flexible alternatives (such as natural splines or polynomials). Except for that, we used natural splines for other variables like after-tax income and net worth of a family unit. While the rule of thumb suggests that we use 5 knots (according to the sample size), our group decided to go with 6 knots to resolve the issue of extreme outlying points that we had with net worth of the family unit.

After the log-transformation, we observe that, though not perfect, the plots have shown more promising results: In figures 11, 12, and 13, the normal Q-Q plot shows an almost straight line, the distribution of error terms is more bell-shaped, and the residual scatter plot is cloud-shaped. We may conclude that the transformation has allowed our assumptions about the model to be reasonably met in order to proceed with our analysis
...
Our analysis shows that there seems to be little to no causal relationship between the total credit limit on all credit cards owned and registered retirement income funds. This contradicts our expectations presented in the causal diagram at the beginning of the report.
